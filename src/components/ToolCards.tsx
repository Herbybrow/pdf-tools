...
// Type-cast Link to resolve Next.js JSX component type mismatch (ts2786 / ts2604)
const NextLink = Link as unknown as React.ComponentType<any>;
...
export function ToolCards({ tools = getCatalogTools() }: ToolCardsProps) {
  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-4 pt-4 pb-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-4 flex items-center justify-start sm:justify-center gap-1.5 sm:gap-2 overflow-x-auto whitespace-nowrap pb-1 sm:pb-0 sm:flex-wrap scrollbar-none [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
        {TOOL_CARD_FILTERS.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                "shrink-0 rounded-full border px-3 py-1.5 text-xs font-medium shadow-sm transition-colors sm:px-4 sm:text-sm",
                selected
